...
void main()
{
	...
	vec3 LightPos0 = vec3( 2,  5, 3);						// Posicion de la luz 0 [fija]
	vec3 LightPos1 = vec3(-4, -5, 3);						// Posicion de la luz 1 [fija]
	...
	vec3  V = normalize(P);	  								// Vector Vision (Eye)
	vec3  R = normalize(reflect(-L, N));					// Vector reflejado R=2N(N.L)-L
	
	float attenuation = 1.0/(0.3+(0.1*d)+(0.01*d*d)); 		// Calculo de la atenuacion
	
	float diffuse  = max(dot(N, L), 0.0);					// Calculo de la intensidad difusa
	...
	V = normalize(P);	  									// Vector Vision (Eye)
	R = normalize(reflect(-L, N));							// Vector reflejado R=2N(N.L)-L
	
	attenuation = 1.0/(0.3+(0.1*d)+(0.01*d*d)); 			// Calculo de la atenuacion
	
	diffuse  = max(dot(N, L), 0.0);							// Calculo de la intensidad difusa
