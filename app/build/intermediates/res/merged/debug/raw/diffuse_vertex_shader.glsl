...
void main()
{
	vec3 LightPos = vec3(0, 1, -7);							// Posicion de la luz [fija]
	//vec3 LightPos = vec3(u_MVMatrix*vec4(0, 1, -3, 1));	// Posicion de la luz [rotando]
	...
	float diffuse = max(dot(N, L), 0.15);					// Calculo de la intensidad difusa (15% de intensidad ambiente)
