...
/* Version 2.5 de Resource3DSReader
*  Changelog:
*    Cambios desde la version 2.0:
*    	Lee grupos de suavizado
*    Cambios desde la version 1.5:
*       Lee multiples mallas del recurso 3DS
*    Cambios desde la version 1.0:
*       Se añade el calculo de la normal y las coordenadas de texturas
*       Cada vertice tiene 8 floats (x, y, z, A, B, C, u, v)
*/
...
public class Resource3DSReader {
	...
	// Numero maximo de mallas de triangulos a leer
	...
	public void analize3DSFromResource(Context context, int resourceId) {
			...
			while (inputStream.available() > 0) {
				...
				switch (l_chunk_id)
			    {
			        ...
			        case CHUNK_FACELIST:
			        	...
			        	//if (LoggerConfig.ON) {
							//Log.w(TAG, "[A] Numero de Poligonos: " + numPol[numMeshes]);
						//}
		...
		if (LoggerConfig.ON) {
			if (numMeshes==1)
				Log.w(TAG, "[A] Recurso 3DS analizado correctamente, con " + numMeshes + " malla y " + totalPol + " poligonos");
			else
				Log.w(TAG, "[A] Recurso 3DS analizado correctamente, con " + numMeshes + " mallas y " + totalPol + " poligonos");
		}
	}
